...
const { filterDBProperty, mapDBToModel } = require('../../utils');
...
class SongsService {
  ...
  async getSongs() {
    ...
    return result.rows.map(filterDBProperty);
  }
  ...
  async getSongDetailById(id) {
    ...
    if (!result.rowCount) {
      throw new NotFoundError('data dengan id tersebut tidak ditemukan');
    }
